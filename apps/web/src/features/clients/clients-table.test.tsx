...
describe('ClientsTable', () => {
  it('shows a placeholder row when there are no accounts', () => {
    render(<ClientsTable clients={[]} onEdit={vi.fn()} onToggleActive={vi.fn()} />)
  ...
  it('renders the acronym next to the name only when set', () => {
    ...
    const { rerender } = render(<ClientsTable clients={[withAcronym]} onEdit={vi.fn()} onToggleActive={vi.fn()} />)
    expect(screen.getByText('(ATL)')).toBeInTheDocument()

    const withoutAcronym = baseClient({ ref: 'CI1', name: 'Marc Dubois', acronym: null })
    rerender(<ClientsTable clients={[withoutAcronym]} onEdit={vi.fn()} onToggleActive={vi.fn()} />)
  ...
  it('dims an inactive account and offers Reactivate instead of Deactivate', () => {
    ...
    render(<ClientsTable clients={[inactive]} onEdit={vi.fn()} onToggleActive={vi.fn()} />)
  ...
  it('does not dim an active account and offers Deactivate instead of Reactivate', () => {
    ...
    render(<ClientsTable clients={[active]} onEdit={vi.fn()} onToggleActive={vi.fn()} />)
  ...
  it('calls onEdit with the clicked client', () => {
    ...
    render(<ClientsTable clients={[client]} onEdit={onEdit} onToggleActive={vi.fn()} />)
  ...
  it('calls onToggleActive with the clicked client', () => {
    ...
    render(<ClientsTable clients={[client]} onEdit={vi.fn()} onToggleActive={onToggleActive} />)
  ...
  it('falls back to an em dash for a missing email, POC phone or billing', () => {
    ...
    render(<ClientsTable clients={[client]} onEdit={vi.fn()} onToggleActive={vi.fn()} />)
