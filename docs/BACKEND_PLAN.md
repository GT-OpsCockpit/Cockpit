...
> **2026-08-27 — Système de permissions généralisé.** `RolesGuard`/`@Roles()` (gate binaire par contrôleur entier, déjà utilisé sur `UsersController`/`CompanyController`) remplacé par `PermissionsGuard`/`@RequirePermission()`, adossé à une carte statique rôle→permissions nommées (`apps/api/src/common/permissions/permissions.ts`) — même mécanisme, mais qui documente *pourquoi* une route est protégée plutôt que juste *par quel rôle*, et permet un check conditionnel en plein milieu d'un service (`can(user, permission)`) là où un guard de route ne peut pas voir le corps de la requête ou l'état existant en DB. Sert de remplacement ISO-legacy au `promptAdminPassword`/`ADMIN_PASSWORD` (secret partagé, sans lien avec l'identité) — remplacé par un vrai contrôle de rôle sur la session déjà authentifiée, sans re-saisie de mot de passe (`POST /api/auth/verify-password` supprimé, devenu obsolète). `GET /auth/me` expose désormais `permissions: Permission[]`, seul canal par lequel le frontend connaît ses droits — voir `docs/agents/permissions.md` pour le détail complet (table de correspondance avec chaque gate legacy, guide pour brancher une nouvelle permission).
